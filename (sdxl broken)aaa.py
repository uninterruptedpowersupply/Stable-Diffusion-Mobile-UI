import os
# Set the environment variable as early as possible.
...
# --- In-Memory Texture Conversion ---
def pil_image_to_texture(pil_image):
    try:
...
def generate_image_with_progress(model_path, prompt, negative_prompt, width, height,
                                 steps, cfg_scale, seed, progress_callback, cancel_event, use_sdxl=False):
    start_time = time.time()
    pipe = None
    cache_key = (model_path, use_sdxl)
    ...
    try:
        ...
        # Normal branch: always generate full image (even for high resolutions)
        with torch.no_grad():
            result = pipe(
                prompt=prompt,
...
class SDApp(App):
    def build(self):
        ...
        debug_btn = Button(text="Show Debug Output", size_hint=(1, None), height=50,
                           background_color=(0.8, 0.8, 0.3, 1))
    ...
    def show_custom_resolution_popup(self, instance):
        ...
        def set_custom_res(instance, wi=width_input, hi=height_input):
            try:
                self.custom_width = int(wi.text)
                self.custom_height = int(hi.text)
    ...
    def run_generation(self):
        try:
            ...
            print("run_generation: output_path =", output_path)
            if output_path:
                try:
                    with Image.open(output_path) as image:
                        texture = pil_image_to_texture(image)
                    Clock.schedule_once(lambda dt: self.update_image(texture, gen_time))
                ...
                print("Output path is None, image not generated.")
        except Exception as e:
    ...
    def update_progress(self, step, total_steps):
        Clock.schedule_once(lambda dt: self._update_progress(step, total_steps))

    def _update_progress(self, step, total_steps):
        self.progress_bar.value = step
        self.progress_label.text = f"Progress: {step}/{total_steps}"

    def update_image(self, texture, gen_time):
